use crate::{db::Db, fsm::FsmHandle, models::AiDifficulty};
...
impl Game {
    ...
    pub async fn on_msg(&mut self, msg: GameMsg, game_handle: GameHandle) {
    ...
    async fn on_play(&mut self, id_user: i32, play: Play, game_handle: GameHandle) {
        ...
        if self.try_play(play, player_num) {
    ...
    fn make_ai_plays(&mut self) {
        ...
        while let Some(to_play) = self.game.to_play() {
            if let Some(ai) = &self.slots[&to_play].ai() {
                ...
                let is_success = self.try_play(play, to_play);
    ...
    fn try_play(&mut self, play: Play, player_num: PlayerNum) -> bool {
