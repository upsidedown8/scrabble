...
use common::game::{bitboard::BitBoard, board::Board};

fn main() {
    // let bb = BitBoard::bottom_row()
    //     | BitBoard::bottom_row() & (BitBoard::leftmost_col() | BitBoard::rightmost_col());
    // let mut bb = bb | BitBoard::leftmost_col();
    let mut bb = BitBoard::zero();
    bb.set_bit(125);

    println!("{}", bb);
    println!("{}", Board::neighbours(bb));
}
